...
// Structure with one constructor per-type of bindings, so that the initializer_list
// accepts bindings with the right type and no extra information.
struct BindingInitializationHelper {
  ...
  // BindingInitializationHelper(uint32_t binding, const wgpu::ExternalTexture&
  // externalTexture);
